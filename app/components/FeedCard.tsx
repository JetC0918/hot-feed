...
import { contrastForeground, sortPosts } from "@/lib/feed-data";
import { SummaryPopover } from "./SummaryPopover";

export function FeedCard({ source, sortMode, authenticated, authEpoch, onRemove }: { source: FeedSource; sortMode: SortMode; authenticated: boolean; authEpoch: number; onRemove?: () => void }) {
  ...
  return <section className="feed-card dashboard-card" aria-labelledby={`feed-${source.id}`}>
    ...
    {posts.length ? <ol className="feed-list">
      {posts.map((post, index) => <li key={post.id} className="post-row">
        <span className="rank" aria-label={`Rank ${index + 1}`} style={{ background: source.color, color: contrastForeground(source.color) }}>{index + 1}</span>
        <SummaryPopover key={`${post.url}-${authEpoch}`} post={post} sourceId={source.id} rank={index + 1} sortMode={sortMode} eligible={authenticated || index < 3}>
