...
import { AUTHENTICATED_SOURCE_IDS, DEFAULT_SOURCE_IDS, FEED_SOURCES, type FeedSource, type SortMode } from "@/lib/feed-data";

const FEED_STORAGE = "hotfeed-selected-sources-v1";
...
export default function Home() {
  ...
  const applyAuthenticatedState = useCallback(() => {
    ...
    try { const raw = window.localStorage.getItem(FEED_STORAGE); if (raw) { const parsed: unknown = JSON.parse(raw); if (Array.isArray(parsed) && parsed.every((id) => typeof id === "string")) saved = parsed; } } catch { /* Ignore invalid device-local preferences. */ }
  ...
  const saveFeeds = (ids: string[]) => { setSelectedIds(ids); window.localStorage.setItem(FEED_STORAGE, JSON.stringify(ids)); };
  ...
  const logout = async () => { await fetch("/api/auth/logout", { method: "POST" }).catch(() => undefined); setAuthenticated(false); setSelectedIds([...DEFAULT_SOURCE_IDS]); };
  ...
  return <div className="app-shell">
    <header className="site-header"><div className="container header-inner">
      ...
      <div className="header-actions">
        ...
        {authenticated ? <><button className="button button-secondary button-small" onClick={refresh} disabled={refreshing}><span aria-hidden="true">↻</span> {refreshing ? "Refreshing…" : "Refresh"}</button><button className="button button-secondary button-small" onClick={logout}><span aria-hidden="true">↪</span> Logout</button></> : <button className="button button-primary button-small" onClick={() => setLoginOpen(true)} disabled={!sessionLoaded}>Sign In</button>}
      </div>
    </div></header>
    ...
    <main className="container main-content">
      <div className={authenticated ? "feed-grid" : "guest-grid"} key={refreshTick}>
        {sources.map((source) => <FeedCard key={source.id} source={source} sortMode={sortMode} authenticated={authenticated} onRemove={authenticated && source.id !== DEFAULT_SOURCE_IDS[0] ? () => removeFeed(source.id) : undefined} />)}
